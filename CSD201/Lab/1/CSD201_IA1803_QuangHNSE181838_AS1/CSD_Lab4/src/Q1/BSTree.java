...
public class BSTree {
    ...
    public int mostExpensivePathCost() {
        return calculateMostExpensivePathCost(root);
    }

    private int calculateMostExpensivePathCost(Node root) {
        if (root == null) {
            return 0;
        }

        int leftCost = calculateMostExpensivePathCost(root.left);
        int rightCost = calculateMostExpensivePathCost(root.right);

        // Choose the maximum cost between the left and right subtrees and add the current node's value
        return Math.max(leftCost, rightCost) + root.info;
    }
    ...
    private boolean checkAVL(Node root) {
        ...
        if (Math.abs(balance) > 1) {
            return false; // If the balance factor is greater than 1, it's not AVL
        }
